...
- get_stock_news: 获取个股近期新闻和公司公告，用于消息面分析
...
5. analyze_stock 已包含近期新闻和公告，分析时须将消息面与技术指标相互印证
6. 用 Markdown 格式输出：表格、列表、加粗关键结论"""
...
TOOL_ANALYZE_STOCK = {
    ...
    "function": {
        "name": "analyze_stock",
        "description": "对单只股票做多周期技术分析。返回日/周/月K的技术指标（MA5/MA20/MA60、MACD、RSI、KDJ、布林带）、基本面、分时数据，以及近期新闻和公司公告。",
        "parameters": {
            "type": "object",
            "properties": {
...
TOOL_GET_STOCK_NEWS = {
    ...
    "function": {
        "name": "get_stock_news",
        "description": "获取个股近期新闻和公司公告。返回新闻标题、来源、发布时间，以及最新公司公告（季报/年报/减持/股权激励等）。用于分析消息面影响。",
        "parameters": {
            "type": "object",
            "properties": {
                "code": {"type": "string", "description": "6位股票代码，如 600519"},
...
ALL_TOOLS = [TOOL_ANALYZE_STOCK, TOOL_GET_STOCK_NEWS, TOOL_SEARCH_STOCKS, TOOL_COMPARE_STOCKS, TOOL_MARKET_BREADTH]
...
def _exec_analyze_stock(args: dict) -> dict:
    ...
    try:
        ...
        # Pick only title/source/time fields, drop full-text content
        news_data = {
            "news": [
                {"title": n.get("新闻标题", ""), "source": n.get("文章来源", ""),
                 "time": str(n.get("发布时间", ""))}
                for n in news_raw
            ],
            "notices": [
                {"title": n.get("公告标题", ""), "type": n.get("公告类型", ""),
                 "date": str(n.get("公告日期", ""))}
                for n in notices_raw
            ],
        }
    except Exception:
        news_data = {"news": [], "notices": []}
...
TOOL_MAP = {
    ...
    "get_stock_news": _exec_get_stock_news,
