...
<template>
  <div class="app-layout">
    ...
    <main class="main-content">
      ...
      <div class="tab-bar">
        <button
          v-for="t in (['agent', 'stocks', 'favorites', 'strategies', 'sectors', 'limit', 'report', 'alerts'] as const)"
          :key="t"
          :class="['tab-btn', { active: activeTab === t }]"
          @click="activeTab = t"
        >{{ { agent: 'AI 选股', stocks: '股票筛选', favorites: '自选', strategies: '策略', sectors: '板块排名', limit: '涨跌停板', report: '日报', alerts: '预警' }[t] }}</button>
      </div>
      ...
      <AlertManager v-else-if="activeTab === 'alerts'" />
    </main>
